...
export function createMobileCardPreview() {
    ...
    const showPeek = () => {
        if (!isMobileCoarse() || !activeCard || !sheet.hidden)
            return;
        peek.hidden = false;
    };
    const renderCard = () => {
    ...
    const closeSheet = () => {
        backdrop.hidden = true;
        sheet.hidden = true;
        if (activeCard && isMobileCoarse())
            showPeek();
    };
    const openSheet = (card) => {
        if (card)
            activeCard = card;
        if (!isMobileCoarse() || !activeCard)
            return;
        renderCard();
        hidePeek();
    ...
    const bindCardGesture = (element, getCard) => {
        let holdTimer = null;
