...
export function createMobileCardInspect() {
    ...
    const showImage = (card) => {
        if (card)
            activeCard = card;
        if (!isMobileInspectEnabled() || !activeCard)
            return;
    ...
    window.addEventListener("resize", () => {
        if (!isMobileInspectEnabled())
            hideAll();
    });
    const bind = (element, getCard) => {
        let pointerId = null;
