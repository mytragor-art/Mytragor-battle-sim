...
function syncLoadoutFromUI() {
    ...
    if (deckId)
        mp.chooseDeck(deckId);
    if (leaderId)
        mp.chooseLeader(leaderId);
}
let isJoining = false;
...
mp.onStateSync = (state) => {
    ...
    $("hand").innerHTML =
        hand
            .map((c) => `<button style="margin:4px;padding:6px;cursor:pointer;" data-card="${c}">${c}</button>`)
            .join("") || "<div>—</div>";
    [...$("hand").querySelectorAll("button")].forEach((btn) => {
        btn.onclick = () => {
