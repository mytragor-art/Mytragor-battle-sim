...
const botDeckEl = document.getElementById("botDeck");
const botLeaderEl = document.getElementById("botLeader");
...
const botDeckLeaderArtEl = document.getElementById("botDeckLeaderArt");
function applySelectedBotDeck(deck) {
    selectedBotDeck = deck;
    if (botLeaderEl)
        botLeaderEl.value = selectedBotDeck?.leaderName || "";
    if (botDeckNameEl)
        botDeckNameEl.textContent = selectedBotDeck?.deckName || "Escolha um deck";
    if (botDeckLeaderEl)
        botDeckLeaderEl.textContent = `Líder: ${selectedBotDeck?.leaderName || "—"}`;
    if (botDeckCardsEl)
        botDeckCardsEl.textContent = selectedBotDeck ? `${selectedBotDeck.cards.length} cartas` : "— cartas";
    ...
    try {
        ...
        bindLobbyHandlers(room, {
            ...
            onLobbyState: (msg) => {
                ...
                const me = Array.isArray(msg?.players) ? msg.players.find((player) => player.slot === mySlot) : null;
                if (view.slotEl) {
...
if (botDeckEl)
    botDeckEl.onchange = syncSelectedBotDeckFromUI;
if (view.btnPreviousDeck)
