...
import { hydrateSavedDecks, readSavedDecks, resolveDeckAssetPath, resolveLeaderArtwork } from "../ui/deckStore";
...
let availableDecks = readSavedDecks();
let deckRefreshToken = 0;
let selectedMatchRoomId = null;
...
function setPrivateCode(code) {
    const normalized = normalizePrivateCode(code);
    if (view.privateCodeEl)
        view.privateCodeEl.value = normalized;
    if (view.privateCodeViewEl)
        view.privateCodeViewEl.textContent = normalized || "—";
    return normalized;
}
...
async function sharePrivateCode() {
    ...
    try {
        ...
        if (error?.name !== "AbortError")
            log("SHARE_ERROR", { error: String(error) });
    }
}
function applySelectedDeck(deck) {
    selectedDeck = deck;
    if (view.leaderViewEl)
        view.leaderViewEl.textContent = selectedDeck?.leaderName || "—";
    if (view.deckCardsCountEl)
        view.deckCardsCountEl.textContent = selectedDeck ? String(selectedDeck.cards.length) : "—";
    if (view.leaderEl)
        view.leaderEl.value = selectedDeck?.leaderName || "";
    if (view.activeDeckNameEl)
        view.activeDeckNameEl.textContent = selectedDeck?.deckName || "Escolha um baralho";
    if (view.activeDeckLeaderEl)
        view.activeDeckLeaderEl.textContent = `Líder: ${selectedDeck?.leaderName || "—"}`;
    if (view.activeDeckCardsEl)
        view.activeDeckCardsEl.textContent = selectedDeck ? String(selectedDeck.cards.length) : "—";
    if (view.activeDeckFragmentLabelEl)
        view.activeDeckFragmentLabelEl.textContent = selectedDeck?.fragImg ? "Fragmento selecionado" : "Fragmento: —";
    ...
    if (view.activeDeckFragmentArtEl) {
        ...
        if (selectedDeck?.fragImg)
            view.activeDeckFragmentArtEl.src = resolveDeckAssetPath(selectedDeck.fragImg);
    }
    if (view.btnPreviousDeck)
        view.btnPreviousDeck.disabled = availableDecks.length < 2;
    if (view.btnNextDeck)
        view.btnNextDeck.disabled = availableDecks.length < 2;
...
async function leaveCurrentLobby() {
    ...
    if (view.roomIdViewEl)
        view.roomIdViewEl.textContent = "—";
}
function endpointToHttpBase(endpoint) {
...
function renderDeckSelector() {
    ...
    if (nextDeck && pendingDeckId === nextDeck.id)
        pendingDeckId = null;
...
function cycleSelectedDeck(direction) {
    if (!view.deckEl || availableDecks.length < 2)
        return;
...
function watchMatch(matchRoomId) {
...
async function joinLobby(forceCreate = false) {
    ...
    try {
        ...
        if (view.roomIdViewEl)
            view.roomIdViewEl.textContent = room.id;
        ...
        bindLobbyHandlers(room, {
            ...
            onLobbyState: (msg) => {
                ...
                const me = Array.isArray(msg?.players) ? msg.players.find((p) => p.slot === mySlot) : null;
                if (view.slotEl) {
...
async function privateCodeExists(code) {
    if (!view.endpointEl)
        return false;
    const base = endpointToHttpBase(view.endpointEl.value.trim());
...
async function enterPrivateLobby(mode) {
    if (isJoining || !view.endpointEl)
        return;
...
if (view.btnJoin) {
    view.btnJoin.onclick = () => {
        if (view.roomIdEl && selectedRoomId && !view.roomIdEl.value.trim())
...
if (view.btnCreatePrivate)
    view.btnCreatePrivate.onclick = () => void enterPrivateLobby("create");
if (view.btnJoinPrivate)
    view.btnJoinPrivate.onclick = () => void enterPrivateLobby("join");
if (view.btnCopyPrivateCode)
    view.btnCopyPrivateCode.onclick = () => void copyPrivateCode();
if (view.btnSharePrivateCode)
    view.btnSharePrivateCode.onclick = () => void sharePrivateCode();
if (view.deckEl)
    view.deckEl.onchange = syncSelectedDeckFromUI;
if (view.btnPreviousDeck)
    view.btnPreviousDeck.onclick = () => cycleSelectedDeck(-1);
if (view.btnNextDeck)
    view.btnNextDeck.onclick = () => cycleSelectedDeck(1);
if (view.btnOpenDeckBuilder)
    view.btnOpenDeckBuilder.onclick = () => { window.location.href = "./ui/deckbuilder.html"; };
if (view.btnEditActiveDeck)
    view.btnEditActiveDeck.onclick = () => {
...
window.addEventListener("storage", (event) => {
    if (event.key && event.key !== "mytragor_decks" && event.key !== "mytragor_play_deck")
        return;
...
if (view.btnRefreshRooms)
    view.btnRefreshRooms.onclick = () => void refreshRooms();
if (view.btnRefreshMatches)
    view.btnRefreshMatches.onclick = () => void refreshMatches();
if (view.btnJoinSelected)
    view.btnJoinSelected.onclick = () => {
        if (!view.roomIdEl || !selectedRoomId)
            return;
        view.roomIdEl.value = selectedRoomId;
        void joinLobby();
    };
...
if (view.endpointEl)
    view.endpointEl.value = params.get("endpoint")?.trim() || resolveServerEndpoint(window.location.search);
if (view.roomIdEl)
