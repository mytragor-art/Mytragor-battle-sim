...
function getArrowSvg() {
    if (!arrowState.svg) {
        arrowState.svg = document.getElementById("attackArrowSvg");
    }
    return arrowState.svg;
}
function getArrowPath() {
    if (!arrowState.path) {
        arrowState.path = document.getElementById("attackArrowPath");
    }
...
function buildCurve(fromX, fromY, toX, toY) {
    const dx = toX - fromX;
    const dy = toY - fromY;
...
function renderArrow() {
    ...
    if (!arrowState.active || !arrowState.fromEl)
        return;
...
export function animateEl(el, className) {
    if (!el)
        return;
...
export function animateCardTransfer(origin, targetEl, options) {
...
export function setChosenReady(el, ready) {
    if (!el)
        return;
...
export function startAttackArrow(fromEl) {
    if (!fromEl)
        return;
    setupAttackArrow();
    const svg = getArrowSvg();
...
export function bindAttackTargetHover(targetEl) {
    if (!targetEl)
        return () => { };
    const handleEnter = () => {
